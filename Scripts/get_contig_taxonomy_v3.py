...
def create_temp_file(input, tempfile):
...
def get_taxonomy(tempfile, output):
    ...
    with open(output, 'w') as o:
        ...
        #I then make a list of all the contigs and, for each one, I check if their proteins have a different taxonomy at any level.
        for cont in contlist:
            number_current_contig += 1
            print(("Starting to work on contig {} of {}").format(str(number_current_contig), str(contnum)))
            contdata = data.where(data['contig'] == cont).dropna()
            smalldata = contdata['annotation'].str.split(';', expand=True)
            smalldata.replace('', pd.np.nan, inplace=True)
            smalldata.replace('Not_assigned', pd.np.nan, inplace=True)
            most_freq_tax = ''
            common_tax = ''
...
if args.get_rank:
    ...
    print('Everything done. The final output is a file named \'' + str(output) + '\' with four columns.')
    print('1: contig names')
    print('2: most frequent taxonomy in the contig')
    print('3: taxonomy common to all proteins on the contig')
    print('4: the proteins on this contig share the same taxonomy? If you find a \"Yes\" in this column, then column 2 and 3 will be the same.')
