...
"""
Step 2:
python prepare_edgeR_input_taxonomy_v2.py -o edgeR_input_taxonomy_NTC_G210_1.contig.order.txt -c NTC_G210.count_tab.matrix -t get_contig_splitted_2_NTC_G210_1.txt -l order
...
if not args.count_table:
    print('A count table file is needed. Please specify it with the -c option.')
    sys.exit()
...
#take the two columns into dict: Contig : get_level
dic = {i[0] : i[1] for i in data[["Contig", "get_level"]].values.tolist()}



# =============================================================================
# for i, cont in enumerate(data[data.columns[0]].tolist()):
#     dic[cont] = get_level_taxonomy(data[data.columns[1]][cont], level)
#     print(str(i)+" / "+str(len(data[data.columns[0]].tolist())))
# 
# =============================================================================
